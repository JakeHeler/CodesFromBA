	...
	public:
	    ...
	    constexpr void reserve( const size_type _Newcapacity) 
	    {
		    ...
		    { 
        			...
        			const pointer _Newvec = _MyAllot.allocate(_Newcapacity);

        			for(int i=0;i < _Size; ++i)
        				construct(_Newvec+i, *(_ArrayStartPtr+i));

            		change_array_pointer(_Newvec,_Size,_Newcapacity);
		    }
	    }
